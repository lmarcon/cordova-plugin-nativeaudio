...
public class NativeAudio extends CordovaPlugin implements AudioManager.OnAudioFocusChangeListener {
    ...
    private PluginResult executeGetDuration(JSONArray data) {
        ...
        try {
            ...
            if (assetMap.containsKey(audioID)) {
                ...
                JSONObject done = new JSONObject();
                done.put("duration", duration);
                return new PluginResult(Status.OK, done);
            } else {
                return new PluginResult(Status.ERROR, ERROR_NO_AUDIOID);
            }
    ...
    private PluginResult executeGetPosition(JSONArray data) {
        final String audioID;
        try {
            audioID = data.getString(0);
            ...
            if (assetMap.containsKey(audioID)) {
                NativeAudioAsset asset = assetMap.get(audioID);
                int position = asset.getPosition();

                JSONObject done = new JSONObject();
                done.put("position", position);
                return new PluginResult(Status.OK, done);
            } else {
                return new PluginResult(Status.ERROR, ERROR_NO_AUDIOID);
            }
    ...
    private PluginResult executeSetPosition(JSONArray data) {
        final String audioID;
        int position;
        try {
            audioID = data.getString(0);
    ...
    public boolean execute(final String action, final JSONArray data, final CallbackContext callbackContext) {
        ...
        try {
            if (SET_OPTIONS.equals(action)) {
                ...
                cordova.getThreadPool().execute(new Runnable() {
                    public void run() {
                        callbackContext.sendPluginResult(executeGetPosition(data));
                    }
                });
            } else if (SET_POSITION.equals(action)) {
                cordova.getThreadPool().execute(new Runnable() {
                    public void run() {
                        callbackContext.sendPluginResult(executeSetPosition(data));
                    }
                });
            } else {
